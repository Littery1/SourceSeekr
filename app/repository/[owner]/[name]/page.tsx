...
useEffect(() => {
  const fetchRepositoryData = async () => {
    try {
      ...
      // Import the fetchRepositoryByFullName function
      const { fetchRepositoryByFullName } = await import("@/lib/github-api");

      // Pass the GitHub token from the user's session
      const githubToken = session?.user?.githubAccessToken || null;
      
      // Fetch the repository data with full GitHub API data
      // Since we've optimized the API usage, we can confidently fetch everything
      // including detailed information like README, contributors, etc.
      const repoData = await fetchRepositoryByFullName(owner, name, githubToken);
  ...
  const toggleSaved = () => {
    setIsSaved(!isSaved);
    // In a real app, you'd call an API to save/unsave the repository
  };
