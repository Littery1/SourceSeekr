...
export default function DashboardClient({ session }: { session: Session }) {
  ...
  const toggleSaveRepo = (repoId: string) => {
    try {
      ...
      localStorage.setItem('sourceseekr-saved', JSON.stringify([...newSavedRepos]));
    } catch (error) {
  ...
  if (rateLimitError) {
    const RateLimitError = dynamic(() => import("@/components/rate-limit-error"), {
      ssr: false,
      loading: () => <p className="text-center py-12">Loading error message...</p>
    });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Link href="/explore" className="card p-6 hover:shadow-md transition-shadow flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Discover new projects with advanced filters</p>
          </div>
        </Link>
        
        <Link href="/saved" className="card p-6 hover:shadow-md transition-shadow flex items-center gap-4">
          <div className="bg-primary/10 p-3 rounded-full">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-primary">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">View your bookmarked projects</p>
          </div>
        </Link>
        
        <Link href="/profile" className="card p-6 hover:shadow-md transition-shadow flex items-center gap-4">
          <div className="bg-primary/10 p-3 rounded-full">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-primary">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Personalize your recommendation settings</p>
          </div>
        </Link>
      </div>

      {/* Profile Completion Banner */}
      {(!userPreferences.preferredLanguages.length || !userPreferences.interests.length) && (
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Set your programming languages and interests to get personalized recommendations
                </p>
              </div>
            </div>
      ...
      {recommendedRepos.length === 0 ? (
        <div className="bg-card rounded-xl border border-border p-8 text-center">
          <p className="text-muted-foreground">No repositories found. Please try again later or update your preferences.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6">
          {recommendedRepos.map((repo) => (
            <div key={repo.id} className="bg-card rounded-xl border border-border shadow-sm hover:shadow-md transition-all duration-300 p-6">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="flex flex-row md:flex-col gap-3 min-w-[130px]">
                  ...
                  <button 
                    ...
                    aria-label={savedRepos.has(repo.id.toString()) ? "Remove from saved" : "Save repository"}
                    title={savedRepos.has(repo.id.toString()) ? "Remove from saved" : "Save repository"}
                  >
                    <svg 
                      xmlns="http://www.w3.org/2000/svg" 
                      viewBox="0 0 24 24" 
                      fill={savedRepos.has(repo.id.toString()) ? "currentColor" : "none"}
                      stroke="currentColor" 
                      className="w-5 h-5"
                    >
                    ...
                    <span className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-card text-foreground text-xs px-2 py-1 rounded shadow opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                      {savedRepos.has(repo.id.toString()) ? "Remove from saved" : "Save repository"}
                    </span>
                  </button>
                </div>
              </div>
      ...
      <div className="mt-12 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-xl p-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-xl font-bold mb-2">AI-Powered GitHub Assistant</h3>
              <p className="text-muted-foreground">
                Chat with our AI assistant to find repositories, get programming advice, and connect with other developers.
              </p>
            </div>
          </div>
