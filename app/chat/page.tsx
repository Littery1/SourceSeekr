...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'system',
        content: 'You are SourceSeekr AI, a helpful assistant that helps users discover GitHub repositories based on their interests and needs. You can access GitHub data and provide recommendations with real-time data from the GitHub API when relevant to the user\'s query, but you can also have regular conversations without using GitHub data when appropriate.',
        timestamp: new Date()
      },
      {
        role: 'assistant',
        content: `Hello${session?.user?.name ? `, ${session.user.name}` : ''}! 👋 I'm your SourceSeekr AI assistant. I can help you find GitHub repositories based on your interests, answer questions about repositories, or provide programming guidance. You can also chat with me about general topics. What are you looking for today?`,
        timestamp: new Date()
      }
    ]);
  }, [session]);
  ...
  useEffect(() => {
    if (rateLimitedResponses.size > 0) {
      try {
        localStorage.setItem('rateLimitedResponses', JSON.stringify(Array.from(rateLimitedResponses)));
      } catch (error) {
        console.error('Error saving rate-limited responses to localStorage:', error);
      }
    }
  ...
  const LoadingIndicator = () => (
    <div className="flex items-center space-x-2 p-4 bg-primary/5 rounded-lg max-w-[80%]">
      <div className="w-4 h-4 rounded-full bg-primary/40 animate-pulse" style={{ animationDelay: "0ms" }}></div>
      <div className="w-4 h-4 rounded-full bg-primary/40 animate-pulse" style={{ animationDelay: "300ms" }}></div>
      <div className="w-4 h-4 rounded-full bg-primary/40 animate-pulse" style={{ animationDelay: "600ms" }}></div>
    </div>
  );
    ...
    <div className="flex flex-col h-[calc(100vh-10rem)] max-w-4xl mx-auto">
      <div className="flex flex-col items-center mb-6">
        ...
        <p className="text-muted-foreground text-center max-w-lg mb-3">
          Ask for repository recommendations, learn about GitHub trends, or get help with your coding journey.
        </p>
        <button 
              ...
              <div className="flex-1">
                {message.role === 'user' ? (
                  ...
                  <>
                    ...
                    {message.role === 'assistant' && rateLimitedResponses.has(message.timestamp.getTime()) && (
                      <div className="mt-2 text-xs bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 px-2 py-1 rounded inline-flex items-center">
                        ...
                        GitHub rate limit exceeded - Responding without GitHub data
                      </div>
                    )}
                  </>
                )}
      ...
      <p className="text-xs text-muted-foreground text-center mt-4">
        SourceSeekr AI uses DeepSeek to provide GitHub repository recommendations with real-time data.
      </p>
    </div>
  );
}